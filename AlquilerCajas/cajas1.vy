...
#Funcion que dado el numero de la caja devuelve un booleano
...
def cambio(ncaja:uint256):
    ...
    assert (caja_.propietario == msg.sender or self.tienda == msg.sender),"Propietario o tienda"
    #Se comprueba que el tiempo de uso es menor que el tiempo actual
    assert ((caja_.tdisfrute < block.timestamp) or (caja_.primera) or (caja_.dejar)),"Posibilidades de llamada"
    ...
    if caja_.ttope < block.timestamp:
        ...
        if caja_.pagada:
            ...
            log Transaccion(self.tienda,caja_.propietario,self.fianza + self.mensualidad)
            self._moroso(ncaja)        
...
def pagar(ncaja: uint256):
    assert self.uso[ncaja],"Caja asignada"
    caja_ :Caja = self.clientes[ncaja]
    assert caja_.propietario == msg.sender,"Propietario"
    assert not caja_.pagada,"No pagada"
    assert msg.value == self.mensualidad,"Valor exacto"
    assert block.timestamp <= caja_.tdisfrute,"Dentro de tiempo"
...
def dejarcaja(ncaja: uint256):
    ...
    assert self.clientes[ncaja].tdisfrute >= block.timestamp,"Dentro de tiempo"
    self.clientes[ncaja].dejar = True
