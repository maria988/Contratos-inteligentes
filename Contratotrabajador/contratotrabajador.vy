...
@external
def firmar():
    assert self.trabajador == msg.sender
    self.trabaja = True
...
@external
def fichar():
    assert msg.sender == self.trabajador
    assert block.timestamp <= self.fin_de_mes
...
def pagar():
    assert msg.sender == self.empresa
    assert (block.timestamp >= self.fin_de_mes) or (not self.trabaja)
    assert (msg.value == self.salario + (self.horas_acumuladas - self.horas_mes)* self.precio_hora+self.mes_indem) or (msg.value == (self.precio_hora * self.horas_acumuladas)+self.mes_indem)
    if self.despedido:
